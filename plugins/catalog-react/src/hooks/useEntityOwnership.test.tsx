...
describe('useEntityOwnership', () => {
  ...
  describe('useEntityOwnership', () => {
    it('matches ownership via token claims', async () => {
      ...
      const rendered = renderHook(() => useEntityOwnership(), {
        wrapper: Wrapper,
      });

      expect(rendered.result.current.loading).toBe(true);
      expect(rendered.result.current.isOwnedEntity(ownedEntity)).toBe(false);

      rendered.rerender();

      await waitFor(() => {
        expect(rendered.result.current.loading).toBe(false);
        expect(rendered.result.current.isOwnedEntity(ownedEntity)).toBe(true);
      });
    });
